...
move = fn stacks, from, to ->
  {val, stacks} = Map.get_and_update(stacks, from, fn
    []      -> {"X", []} # shouldn't happen, just for debug
    [h | t] -> {h, t}
  end)
...
res =
  ...
  |> Enum.reduce(stacks, fn [times, from, to], s ->
    (1 .. times) |> Enum.reduce(s, fn _, acc -> move.(acc, from, to) end)
  end)
