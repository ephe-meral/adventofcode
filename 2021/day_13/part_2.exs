...
end_points = folds |> Enum.reduce(points, fn fld, pts -> fold.(pts, fld) end)

max_x = end_points |> List.flatten() |> Enum.take_every(2) |> Enum.max()
max_y = end_points |> List.flatten() |> Enum.drop(1) |> Enum.take_every(2) |> Enum.max()

Enum.map(0..max_y, fn y ->
  Enum.map(0..max_x, fn x -> ([[x], [y]] in end_points) && "#" || " " end)
  |> Enum.join("")
  |> IO.puts()
end)
